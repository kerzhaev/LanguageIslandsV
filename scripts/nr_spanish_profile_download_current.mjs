...
if (!targetDirs.length) {
  console.error("Usage: node scripts/nr_spanish_profile_download_current.mjs <target-dir> [<target-dir>...]");
  process.exit(1);
}
...
async function readJob(targetDir) {
  ...
  if (!onceInput || !repeatInput) {
    throw new Error(`Spanish NaturalReaders inputs not found in ${targetDir}`);
  }
  ...
  return {
    ...
    onceMp3: path.join(
      targetDir,
      onceInput.replace("__04a__shadowing_es__naturalreaders_input.txt", "__04__shadowing_es.mp3"),
    ),
    repeatMp3: path.join(
      targetDir,
      repeatInput.replace("__06a__shadowing_es_repeat__naturalreaders_input.txt", "__06__shadowing_es_repeat.mp3"),
    ),
  };
}
